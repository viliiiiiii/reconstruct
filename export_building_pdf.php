...
// Set QR size to approximately 3x3 cm (85px at 72 DPI, but we'll use 100px for better quality)
$qrSize  = 100; // Fixed size for 3x3 cm
...
<style>
  @page { 
    size: A4; 
    margin: 5mm; /* Minimal margins for A4 paper */
  }
  body {
  ...
  .badge {
    ...
    padding:6px 12px; border-radius:999px; /* Increased padding */
    background:rgba(59,130,246,.12); color:#1e3a8a;
    font-weight:700; font-size:11px; letter-spacing:.05em; text-transform:uppercase; /* Increased font size */
  }
  .cover {
    padding: 25px 22px 30px; /* Increased padding */
    border-radius: 16px;
  ...
  .cover__title {
    font-size: 26px; font-weight: 800; letter-spacing: .12em; text-transform: uppercase; /* Increased from 22px */
    color: #0b1d4d;
  }
  .cover__subtitle {
    margin-top: 6px; font-size: 14px; color: #334155; letter-spacing: .22em; /* Increased from 12px */
    text-transform: uppercase;
  }
  .cover__meta { margin-top: 16px; display:flex; flex-wrap:wrap; gap:12px 20px; font-size: 12px; color:#0f172a; } /* Increased font size */
  .cover__meta span { font-weight:700; }
  .pill-row { margin-top: 20px; display:flex; flex-wrap:wrap; gap:10px; }
  .pill {
    display:flex; flex-direction:column; align-items:flex-start;
    padding:10px 14px; border-radius:12px; /* Increased padding */
    background:linear-gradient(135deg, rgba(59,130,246,.12), rgba(14,165,233,.08));
    border:1px solid rgba(148,163,184,.18);
  }
  .pill strong { font-size:14px; } /* Increased from 12px */
  .pill small { color:#475569; font-size:11px; text-transform:uppercase; letter-spacing:.08em; } /* Increased font size */
  ...
  .room-meta { display:flex; flex-wrap:wrap; gap:12px 16px; font-size:11px; text-transform:uppercase; letter-spacing:.08em; color:#475569; } /* Increased font size */
  .meta-value { font-weight:700; color:#0f172a; }
  .qr-wrapper {
    text-align:center; 
    min-width: 120px; /* Reduced from dynamic calculation */
  }
  .qr-halo {
    display:inline-block; padding:8px; border-radius:12px; /* Reduced padding */
    background:linear-gradient(160deg, rgba(15,23,42,.08), rgba(148,163,184,.12));
    box-shadow:0 8px 16px rgba(15,23,42,.12);
  }
  .qr-halo img { 
    display:block; 
    width: 100px; /* Fixed 3x3 cm size */
    height: 100px; /* Fixed 3x3 cm size */
  }
  .qr-caption { margin-top:6px; font-size:10px; color:#475569; letter-spacing:.1em; text-transform:uppercase; } /* Increased font size */

  .task-table { width:100%; border-collapse:collapse; margin-top:16px; }
  .task-table th {
    background:#e2e8ff; color:#1e3a8a; font-size:12px; letter-spacing:.1em; text-transform:uppercase; /* Increased from 9px */
    padding:10px 8px; text-align:left; border-bottom:2px solid rgba(148,163,184,.35); /* Increased padding */
  }
  .task-table td {
    padding:9px 8px; border-bottom:1px solid rgba(148,163,184,.25); vertical-align:top; font-size:12px; /* Increased from 9px */
  }
  ...
  .empty-note { margin-top:16px; padding:14px; border-radius:10px; border:1px dashed rgba(148,163,184,.4); background:rgba(248,250,255,.8); color:#475569; font-size:12px; } /* Increased font size */
</style>
</head>
<body>
  <section class="cover">
    ...
    <?php if ($summary): ?>
      <div class="empty-note" style="margin-top:16px;">Filters: <?php echo htmlspecialchars($summary, ENT_QUOTES, 'UTF-8'); ?></div>
    <?php endif; ?>
  ...
  ?>
    <section class="room-section">
      <header class="room-header">
        <div>
          <div class="building-name"><?php echo htmlspecialchars($buildingName, ENT_QUOTES, 'UTF-8'); ?></div>
...
$cmd = sprintf(
  ...
  '--margin-top 5mm --margin-right 5mm --margin-bottom 5mm --margin-left 5mm ' . // Minimal margins
  '--page-size A4 --orientation Portrait ' .
  '--footer-right "Page [page] of [toPage]" --footer-font-size 10 ' . // Increased footer font size
  '%s %s %s 2>&1',
